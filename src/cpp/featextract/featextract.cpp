...
PyObject* swap_axes(PyObject* cost ){
    ...
    _MY_FLOAT_ * costp = reinterpret_cast<_MY_FLOAT_*>(PyArray_DATA(costA));
	...
	PyObject* res = PyArray_SimpleNew(3, res_shape, NPY_FLOAT32);

	_MY_FLOAT_* res_data = static_cast<_MY_FLOAT_*>(PyArray_DATA(reinterpret_cast<PyArrayObject*>(res)));
...
PyObject* get_samples(
	...
	_MY_FLOAT_ * volp = reinterpret_cast<_MY_FLOAT_*>(PyArray_DATA(volA));
	...
	PyObject* res= PyArray_SimpleNew(1, sshape, NPY_FLOAT32);
	_MY_FLOAT_* resd = static_cast<_MY_FLOAT_*>(PyArray_DATA(reinterpret_cast<PyArrayObject*>(res)));
...
PyObject* extract_pkrn_test(
	...
	PyObject* res= PyArray_SimpleNew(2, vol_shape, NPY_FLOAT32);
	_MY_FLOAT_* resd = static_cast<_MY_FLOAT_*>(PyArray_DATA(reinterpret_cast<PyArrayObject*>(res)));
...
#pragma omp parallel num_threads(THREADS_NUM_USED)
	{
		...
		for (long int i= 0; i <vol_shape[0]; i++ ){
			const unsigned long int pos = i* vol_shape[1];
			_MY_FLOAT_ min_cost = RAND_MAX,denom=0,num = 0;
			...
			_MY_FLOAT_ tmp_cost = volp[pos+r_sampp[3*i]] - min_cost;
...
#pragma omp parallel num_threads(THREADS_NUM_USED)
	{
		...
		for (long int i= 0; i <vol_shape[0]; i++ ){
			...
			for(int j=0; j< vol_shape[1]; j++){
				...
				_MY_FLOAT_ tmp_cost = volp[pos+j] - min_cost;
